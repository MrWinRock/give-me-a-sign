...
public class AnomalySpawnManager : MonoBehaviour
{
    ...
    private List<AnomalySpawnEntry> scheduledEntries = new List<AnomalySpawnEntry>();
    ...
    private void InitializeSystem()
    {
        ...
        if (showDebugInfo)
        {
            Debug.Log($"AnomalySpawnManager: Initialized with {scheduledEntries.Count} scheduled spawns.");
        }
    }
    ...
    private void SetupSpawnSchedule()
    {
        ...
        scheduledEntries = spawnList.GetSortedEntries();
        spawnList.ResetAllEntries();
        
        // Remove invalid entries
        for (int i = scheduledEntries.Count - 1; i >= 0; i--)
        {
            if (!scheduledEntries[i].IsValid())
            {
                if (showDebugInfo)
                {
                    Debug.LogWarning($"Removing invalid entry: {scheduledEntries[i].entryName}");
                }
                scheduledEntries.RemoveAt(i);
            }
        }
        
        if (showDebugInfo)
        {
            Debug.Log($"Spawn schedule setup complete. {scheduledEntries.Count} valid entries scheduled.");
        }
    }
    
    private void CheckForSpawns(float normalizedTime)
    {
        if (!isInitialized || scheduledEntries == null) return;
        
        // Check each scheduled entry
        for (int i = scheduledEntries.Count - 1; i >= 0; i--)
        {
            var entry = scheduledEntries[i];
            
            if (!entry.hasSpawned && normalizedTime >= entry.GetNormalizedSpawnTime())
            {
                SpawnAnomaly(entry);
                scheduledEntries.RemoveAt(i); // Remove from schedule after spawning
            }
        }
        
        // Check if all anomalies have been spawned
        if (scheduledEntries.Count == 0 && spawnedAnomalies.Count > 0)
        {
            OnAllAnomaliesSpawned?.Invoke(spawnedAnomalies.Count);
    ...
    private void SpawnAnomaly(AnomalySpawnEntry entry)
    {
        ...
        spawnedAnomalies.Add(spawnedAnomaly);
    ...
    private void OnNightEnded()
    {
        if (showDebugInfo)
        {
            Debug.Log($"Night ended. Spawned {spawnedAnomalies.Count} anomalies total.");
        }
    ...
    private void CleanupSpawnedAnomalies()
    {
        ...
        for (int i = spawnedAnomalies.Count - 1; i >= 0; i--)
        {
            if (spawnedAnomalies[i] == null)
            {
                spawnedAnomalies.RemoveAt(i);
            }
        }
    }
    ...
    {
        CleanupSpawnedAnomalies();
        return spawnedAnomalies.Count;
    }
    
    public List<GameObject> GetSpawnedAnomalies()
    {
        CleanupSpawnedAnomalies();
        return new List<GameObject>(spawnedAnomalies);
    }
    
    public int GetRemainingSpawns()
    {
        return scheduledEntries != null ? scheduledEntries.Count : 0;
    }
    ...
    {
        if (scheduledEntries != null && scheduledEntries.Count > 0)
        {
            SpawnAnomaly(scheduledEntries[0]);
    ...
    public void ForceSpawnAll()
    {
        ...
        {
            while (scheduledEntries.Count > 0)
            {
                SpawnAnomaly(scheduledEntries[0]);
                scheduledEntries.RemoveAt(0);
            }
